...
namespace Core.Test
{
    public class SpanningTreeKruskalTests
    {
        [Fact]
        public static void GivenGraphFindSpanningTreeUsingKruskalAlg()
        {
            var g = ExampleGraph.Create(isDirected: false);

            var spanningTreeEdges = Algorithm.SpanningTreeKruskal(g)
                .AsEnumerable()
                .ToList();
